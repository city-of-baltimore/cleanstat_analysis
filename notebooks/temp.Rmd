...
sr_geo <- SpatialPointsDataFrame(
  coords = sr %>% select(longitude, latitude) %>% filter(!is.na(latitude)), 
  data = sr %>% filter(!is.na(latitude)), 
  proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
)

wo_geo <- SpatialPointsDataFrame(
  coords = wo %>% select(Longitude, Latitude) %>% filter(!is.na(Latitude)), 
  data = wo %>% filter(!is.na(Latitude)), 
  proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
)
...
wo_geo@data <- wo_geo@data %>%
  bind_cols(over(wo_geo, quads))
```
...
sr_2019 <- sr_geo %>%
  filter(year(sr_geo$createddate) == 2019,
         !is.na(closedate),
         !is.na(ID),
         !grepl("Duplicate", srstatus),
         !grepl("Transferred", srstatus),
         srtype == "SW-Dirty Alley") %>%
  mutate(Overdue = ifelse(closedate > duedate, T, F))
