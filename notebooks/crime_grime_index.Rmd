...
hood_metrics_over150@data <- hood_metrics_over150@data %>%
  ...
  ungroup() %>%
  mutate(crime_index = 0.5 * homicide_shootings_index + 
           0.5 * other_violent_crime_index,
         crime_grime_index = 0.5 * crime_index + 0.5 * grime_index)
  
hood_metrics_over150@data <- hood_metrics_over150@data %>%
  mutate(crime_grime_index_quadrature = sqrt(crime_index^2 + grime_index^2))
```
...
$$ CrimeGrimeIndex = \frac{1}{2}(CrimeIndex +GrimeIndex) $$
...
pal <-  colorNumeric("viridis", domain = hoods$crime_grime_index)
...
#write_csv(the_list, "../data/processed/crime_grime_index_v1.csv")
